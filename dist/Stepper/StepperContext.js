...
export const StepperContext = /*#__PURE__*/React.createContext({
  activeKey: ''
});
  ...
  switch (action.type) {
    ...
    default:
      ...
      if (stepsState.some(step => step.index)) {
        return newStepsState.sort((a, b) => a.index > b.index ? 1 : -1);
      }
...
export function StepperContextProvider({
  ...
  const removeStep = useCallback(eventKey => dispatch({
    eventKey,
    type: 'remove'
  }), []);
